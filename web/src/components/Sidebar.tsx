...
import {
  ...
  FileText, DollarSign, ChevronLeft, ChevronRight, Zap, X,
} from 'lucide-react';
...
export function Sidebar({ mobileOpen = false, onClose }: SidebarProps) {
  ...
  const sidebarContent = (isMobile: boolean) => (
    <div
      ...
      style={{
        ...
        ...(!isMobile && { transition: 'width 0.25s ease' }),
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-3 min-h-[60px]">
        ...
        {(isMobile || !collapsed) && (
          <span className="font-bold text-slate-100 text-base tracking-tight whitespace-nowrap flex-1">
            AdEarn
          </span>
        )}
        ...
        {isMobile && (
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-200 hover:bg-white/[0.08] transition-colors shrink-0"
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex flex-col h-full shrink-0" style={{ flexShrink: 0 }}>
